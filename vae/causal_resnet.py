...
class CausalResConv1DBlock(nn.Module):
    def __init__(self, n_in, n_state, dilation=1, activation='silu', norm=None, dropout=None, pad_mode='reflect'):
        ...
        p = 0.0 if dropout is None else float(max(0.0, min(1.0, dropout)))
        self.dropout = nn.Dropout(p) if p > 0.0 else nn.Identity()
...
class CausalResnet1D(nn.Module):
    def __init__(self, n_in, n_depth, dilation_growth_rate=1, reverse_dilation=True, activation='relu', norm=None, dropout=None, pad_mode='replicate'):
        super().__init__()
